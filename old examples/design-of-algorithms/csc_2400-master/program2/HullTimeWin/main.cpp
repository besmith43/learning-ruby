...
//for some reason, the other timer won't work on win8...
#include "HighPerformanceCounter.h"
#include "HighPerformanceCounter.cpp"
...
//have to define stoi for windows, even though it's in the standard library somewhere...
#if defined(_WIN32)
...
int main(int argc, const char *argv[]){
   ...
   //get timer
   HighPerformanceCounter *timer = HighPerformanceCounter::getHighPerformanceCounter();
   ...
   for (int i = startNum; i <= grid->getSize(); i = i + stepSize){
      ...
      int startTime = timer->getCurrentTimeInTicks();
      
      grid->bruteForceHull(i);
      
      int endTime = timer->getCurrentTimeInTicks();
      
      double elapsedTime = timer->getTimeDifferenceInMicroSeconds(startTime, endTime);
