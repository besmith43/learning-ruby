...
module PrefsWindowTests
  class PreferencesTableViewTests < Test::Unit::TestCase
    ...
    context "checking a location where a drop is possible" do
      setup do
        @info = rubycocoa_flexmock("drop info")  #(1) 
    ...
    context "checking a location where a drop is not possible" do
      ...
      should_eventually "leave no row selected" do
        @sut.fenestra.evaluate_location(@info)
        assert { @sut.numberOfSelectedRows == 0 }
      end

      should_eventually "signal that no operation is permitted" do 
        retval = @sut.fenestra.evaluate_location(@info)
    ...
    context "exiting dragging" do
      should_eventually "leave no rows highlighted" do
        @sut.select_row(1)
        @sut.fenestra.forget_drag('ignored')  # (2) 
        assert { @sut.numberOfSelectedRows == 0 }
      end
    end
