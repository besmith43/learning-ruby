      ...
      context "in cases that do not launch file browser," do 
        ...
        context "clicking on non-source column" do 
          should 'still perform normal editing' do
            during_doubleclick_on(@display_name_column_index, 1).behold! { # (1) 
              @table.should_receive(:editColumn_row_withEvent_select, 4).once. # (2)
                     with(@display_name_column_index, 1, any, 1)
              watchers_are_notified.never # (3)
            }
            assert { @original_sources == self.current_sources }  # (4)
          end
        end
        ...
        context "clicking outside normal bounds" do
          should_eventually 'do nothing (if out of row bounds)' do
            during_doubleclick_on(@source_column_index, -1).behold! {  # (5)
              nothing_happens  # (6)
            }
          ...
          should_eventually 'do nothing (if out of column bounds)' do
            during_doubleclick_on(-1, 1).behold! {  # (7)
              nothing_happens  # (8)
            }
            assert { @original_sources == self.current_sources } 
          end
